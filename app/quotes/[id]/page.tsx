...
export default function QuoteDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const statusLabels: Record<string, string> = lang === 'zh'
    ...
    <div className="min-h-screen bg-gray-100">
      <div className="max-w-4xl mx-auto p-6 print:hidden space-y-3">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-semibold text-gray-900">{lang === 'zh' ? '报价单详情' : 'Quote Detail'}</h1>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4 flex gap-2 flex-wrap items-center">
          <span className="text-sm text-gray-500">{lang === 'zh' ? '状态' : 'Status'}:</span>
      ...
      <div className="max-w-4xl mx-auto bg-white p-8 shadow-sm print:p-6">
        ...
        <table className="w-full border-collapse mb-0 text-sm">
          <tbody>
            <tr className="border border-gray-300">
              <td className="border border-gray-300 px-3 py-1 w-28 text-gray-500">{lang === 'zh' ? '报价单号' : 'Quote No.'}</td>
              <td className="border border-gray-300 px-3 py-1 font-medium">{quote.quote_number || 'Q-001'}</td>
              <td className="border border-gray-300 px-3 py-1 w-28 text-gray-500">{lang === 'zh' ? '日期' : 'Date'}</td>
              <td className="border border-gray-300 px-3 py-1">{quote.quote_date || new Date().toLocaleDateString('en-AU')}</td>
            </tr>
            <tr className="border border-gray-300">
              <td className="border border-gray-300 px-3 py-1 text-gray-500">{lang === 'zh' ? '类型' : 'Type'}</td>
              <td className="border border-gray-300 px-3 py-1 font-medium">{quote.quote_type || 'Residential'}</td>
              <td className="border border-gray-300 px-3 py-1 text-gray-500">{lang === 'zh' ? '建筑商' : 'Builder'}</td>
              <td className="border border-gray-300 px-3 py-1">{quote.builder_name || ''}</td>
            </tr>
            <tr className="border border-gray-300">
              <td className="border border-gray-300 px-3 py-1 text-gray-500">{lang === 'zh' ? '地址' : 'Address'}</td>
        ...
        <table className="w-full border-collapse text-sm mt-4">
          <thead>
            <tr className="bg-gray-100">
              ...
              <th className="border border-gray-400 px-2 py-2 text-left">{lang === 'zh' ? '名称' : 'Name'}</th>
              <th className="border border-gray-400 px-2 py-2 text-left w-16">{lang === 'zh' ? '类型' : 'Type'}</th>
              <th className="border border-gray-400 px-2 py-2 text-left w-12">{lang === 'zh' ? '单位' : 'Unit'}</th>
              <th className="border border-gray-400 px-2 py-2 text-right w-12">{lang === 'zh' ? '数量' : 'Qty'}</th>
              <th className="border border-gray-400 px-2 py-2 text-right w-24">{lang === 'zh' ? '单价' : 'Rate'}</th>
              <th className="border border-gray-400 px-2 py-2 text-right w-24">{lang === 'zh' ? '金额' : 'Amount'}</th>
            </tr>
          </thead>
          <tbody>
            {items.map((item) => (
        ...
        <div className="flex justify-end mt-2">
          <table className="border-collapse text-sm">
            <tbody>
              <tr className="border border-gray-300">
                <td className="border border-gray-300 px-6 py-1 font-medium">{lang === 'zh' ? '小计' : 'Sub-Total'}</td>
              ...
              <tr className="border border-gray-300 bg-blue-50">
                <td className="border border-gray-300 px-6 py-1 font-bold">{lang === 'zh' ? '含GST总计' : 'Total Inc GST'}</td>
        ...
        {quote.scope_of_work && (
          <div className="mt-6 border border-gray-300 p-4">
            <p className="font-bold text-sm mb-2">{lang === 'zh' ? '工作范围' : 'General Scope of Work'}:</p>
