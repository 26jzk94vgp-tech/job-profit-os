        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
              ...
              <tbody>
                {items.map((item, idx) => (
                  <tr key={item.id}>
                    ...
                    <td className="border border-gray-200 px-2 text-right text-gray-600 dark:text-gray-400">${(Number(item.quantity) * Number(item.unit_price) || 0).toFixed(2)}</td>
          ...
          <div className="flex justify-end text-sm space-y-1">
            <div className="text-right space-y-1">
              <p className="text-gray-600 dark:text-gray-400">{lang === 'zh' ? '小计' : 'Sub-Total'}: <span className="font-medium">${subTotal.toFixed(2)}</span></p>
              <p className="text-gray-600 dark:text-gray-400">GST: <span className="font-medium">${gst.toFixed(2)}</span></p>
