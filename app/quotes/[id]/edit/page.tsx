              ...
              <tbody>
                {items.map((item, idx) => (
                  <tr key={item.id} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
                    ...
                    <td className="px-1 py-1"><input list="cimoUnits" className={cellInputCls} value={item.item_unit || ''} onChange={(e) => updateItem(idx, 'item_unit', e.target.value)} />{idx === 0 && <datalist id="cimoUnits"><option value="sqm">平方米</option><option value="qbm">立方米</option><option value="L">升</option><option value="KL">千升</option><option value="KG">千克</option><option value="LM">米</option><option value="EA">个</option></datalist>}</td>
                    <td className="px-1 py-1"><input type="number" disabled={item.priceMode === 'total'} className={cellInputCls + ' text-right'} value={item.quantity} onChange={(e) => updateItem(idx, 'quantity', e.target.value)} /></td>
                    ...
                    <td className="px-1 py-1"><div className="flex gap-0.5 mb-1 justify-end"><button type="button" onClick={() => updateItem(idx, 'priceMode', 'unit')} className={item.priceMode !== 'total' ? 'px-1.5 py-0.5 rounded text-[10px] font-semibold bg-[#0A84FF] text-white' : 'px-1.5 py-0.5 rounded text-[10px] font-semibold bg-gray-200 dark:bg-[#3A3A3C] text-gray-500'}>单</button><button type="button" onClick={() => { updateItem(idx, 'priceMode', 'total'); updateItem(idx, 'quantity', '1') }} className={item.priceMode === 'total' ? 'px-1.5 py-0.5 rounded text-[10px] font-semibold bg-[#E3B341] text-black' : 'px-1.5 py-0.5 rounded text-[10px] font-semibold bg-gray-200 dark:bg-[#3A3A3C] text-gray-500'}>总</button></div><input type="number" className={cellInputCls + ' text-right'} value={item.unit_price} onChange={(e) => updateItem(idx, 'unit_price', e.target.value)} /></td>
