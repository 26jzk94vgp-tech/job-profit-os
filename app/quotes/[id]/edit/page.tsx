    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <nav className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="max-w-4xl mx-auto flex items-center gap-3">
          ...
          <h1 className="font-semibold text-gray-900 dark:text-white">{lang === 'zh' ? '编辑报价单' : 'Edit Quote'}</h1>
        </div>
      </nav>
      <main className="max-w-4xl mx-auto px-6 py-8 space-y-6">

        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
          <h2 className="font-semibold text-gray-900 dark:text-white">{lang === 'zh' ? '基本信息' : 'Basic Info'}</h2>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
          <div className="flex justify-between items-center">
            <h2 className="font-semibold text-gray-900 dark:text-white">{lang === 'zh' ? '条目' : 'Items'}</h2>
              ...
              <tbody>
                {items.map((item, idx) => (
                  <tr key={item.id}>
                    ...
                    <td className="border border-gray-200 px-2 text-right text-gray-600 dark:text-gray-400 dark:text-gray-500">${(Number(item.quantity) * Number(item.unit_price) || 0).toFixed(2)}</td>
          ...
          <div className="flex justify-end text-sm space-y-1">
            <div className="text-right space-y-1">
              <p className="text-gray-600 dark:text-gray-400 dark:text-gray-500">{lang === 'zh' ? '小计' : 'Sub-Total'}: <span className="font-medium">${subTotal.toFixed(2)}</span></p>
              <p className="text-gray-600 dark:text-gray-400 dark:text-gray-500">GST: <span className="font-medium">${gst.toFixed(2)}</span></p>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
          <h2 className="font-semibold text-gray-900 dark:text-white">{lang === 'zh' ? '工作范围' : 'Scope of Work'}</h2>
