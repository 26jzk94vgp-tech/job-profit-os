    ...
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="max-w-2xl mx-auto flex items-center gap-3">
          ...
          <h1 className="font-semibold text-gray-900">New Quote</h1>
        </div>
      </nav>
      <main className="max-w-2xl mx-auto px-6 py-8">
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
          ...
          <div>
            <label className="text-gray-700 text-sm font-medium">Job</label>
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              <label className="text-gray-700 text-sm font-medium">Items</label>
              <button onClick={addItem} className="text-blue-600 text-sm">+ Add Item</button>
            </div>
            <p className="text-gray-400 text-xs mb-3">Tip: Use / for unknown values</p>
            <div className="space-y-3">
              {items.map((item, index) => (
                <div key={index} className="border border-gray-200 rounded-xl p-4 space-y-2">
                  ...
                  <input className="w-full border border-gray-200 rounded-lg p-2 text-gray-900 outline-none text-sm" placeholder="Description" value={item.description} onChange={(e) => updateItem(index, 'description', e.target.value)} />
                  <div className="flex gap-2">
                    ...
                    <input className="flex-1 border border-gray-200 rounded-lg p-2 text-gray-900 outline-none text-sm" placeholder="Price $ (or /)" value={item.unit_price} onChange={(e) => updateItem(index, 'unit_price', e.target.value)} />
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <textarea className="w-full border border-gray-200 rounded-lg p-3 mt-1 text-gray-900 outline-none" rows={3} placeholder="e.g. Payment due within 14 days" value={notes} onChange={(e) => setNotes(e.target.value)} />
          </div>
          <button onClick={handleSubmit} disabled={loading} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-lg font-medium disabled:opacity-50">{loading ? 'Saving...' : 'Create Quote'}</button>
        </div>
      </main>
    </div>
  )
