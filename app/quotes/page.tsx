    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200 dark:border-gray-700">
          {!quotes.length && <div className="px-6 py-16 text-center text-gray-400 dark:text-gray-500">{lang === 'zh' ? '还没有报价单' : 'No quotes yet.'}</div>}
            ...
            {quotes.map((quote) => (
              <Link href={"/quotes/" + quote.id} key={quote.id}>
                <div className="px-6 py-4 flex justify-between items-center hover:bg-gray-50 transition cursor-pointer">
                  <div>
                    <p className="font-medium text-gray-900 dark:text-white">{quote.clients?.name || (lang === 'zh' ? '无客户' : 'No client')}</p>
