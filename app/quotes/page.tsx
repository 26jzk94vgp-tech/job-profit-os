    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#1C1C1E]">
      <nav className="bg-white border-b border-gray-200 px-6 py-4 hidden md:block">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-semibold text-gray-900 dark:text-[#F2F2F7]">{lang === 'zh' ? '报价单' : 'Quotes'}</h1>
          </div>
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200 dark:border-[#3A3A3C]">
          {!quotes.length && <div className="px-6 py-16 text-center text-gray-400 dark:text-[#636366]">{lang === 'zh' ? '还没有报价单' : 'No quotes yet.'}</div>}
            ...
            {quotes.map((quote) => (
              <Link href={"/quotes/" + quote.id} key={quote.id}>
                <div className="px-6 py-4 flex justify-between items-center hover:bg-gray-50 transition cursor-pointer">
                  <div>
                    <p className="font-medium text-gray-900 dark:text-[#F2F2F7]">{quote.clients?.name || (lang === 'zh' ? '无客户' : 'No client')}</p>
