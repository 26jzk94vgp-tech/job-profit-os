...
export default function Invoice({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="invoice-light-mode min-h-screen" style={{backgroundColor: '#F2F2F7', colorScheme: 'light'}}>
      ...
      <div className="max-w-2xl mx-auto px-4 pt-6 pb-8 print:hidden">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="font-semibold text-base" style={{color: '#1C1C1E'}}>{lang === 'zh' ? '发票' : 'Invoice'}</h1>
        </div>

        {importDone && (
          <div className="rounded-2xl px-4 py-3 mb-4" style={{backgroundColor: '#E8F8ED', border: '1px solid #30D158'}}>
            <p className="text-sm font-medium" style={{color: '#30D158'}}>✅ {lang === 'zh' ? '已自动从报价单导入细分条目' : 'Quote items imported automatically'}</p>
          </div>
        )}

        {/* 发票信息卡片 */}
        <div className="rounded-2xl shadow-sm overflow-hidden mb-4" style={{backgroundColor: '#FFFFFF'}}>
          <div className="grid grid-cols-2" style={{borderBottom: '1px solid #F2F2F7'}}>
            <div className="px-4 py-3" style={{borderRight: '1px solid #F2F2F7'}}>
              <p className="text-xs mb-1" style={{color: '#8E8E93'}}>{lang === 'zh' ? '发票编号' : 'Invoice No.'}</p>
              <input className="w-full bg-transparent outline-none text-sm font-semibold" style={{color: '#1C1C1E'}} value={invoiceNumber} onChange={e => setInvoiceNumber(e.target.value)} />
            </div>
            <div className="px-4 py-3">
              <p className="text-xs mb-1" style={{color: '#8E8E93'}}>{lang === 'zh' ? '到期日' : 'Due Date'}</p>
              <input type="date" className="w-full bg-transparent outline-none text-sm" style={{color: '#1C1C1E'}} value={dueDate} onChange={e => setDueDate(e.target.value)} />
            </div>
          </div>
          <div className="px-4 py-3" style={{borderBottom: '1px solid #F2F2F7'}}>
            <p className="text-xs mb-1" style={{color: '#8E8E93'}}>{lang === 'zh' ? '账单送达' : 'Bill To'}</p>
            <input className="w-full bg-transparent outline-none text-sm font-medium" style={{color: '#1C1C1E'}} placeholder={lang === 'zh' ? '客户名称' : 'Client name'} value={toName} onChange={e => setToName(e.target.value)} />
          </div>
          <div className="px-4 py-3">
            <p className="text-xs mb-1" style={{color: '#8E8E93'}}>{lang === 'zh' ? '客户地址' : 'Address'}</p>
            <input className="w-full bg-transparent outline-none text-sm" style={{color: '#8E8E93'}} placeholder="e.g. 123 Smith St, Perth WA" value={toAddress} onChange={e => setToAddress(e.target.value)} />
          </div>
        </div>
        ...
        <div className="rounded-2xl shadow-sm overflow-hidden mb-4" style={{backgroundColor: '#FFFFFF'}}>
          <div className="px-4 py-3" style={{borderBottom: '1px solid #F2F2F7'}}>
            <p className="text-xs mb-1" style={{color: '#8E8E93'}}>{lang === 'zh' ? '客户邮箱' : 'Client Email'}</p>
            <input type="email" className="w-full bg-transparent outline-none text-sm" style={{color: '#1C1C1E'}} placeholder="client@email.com" value={toEmail} onChange={e => setToEmail(e.target.value)} />
          </div>
          <div className="px-4 py-3">
            <p className="text-xs mb-1" style={{color: '#8E8E93'}}>{lang === 'zh' ? '备注 / 付款条款' : 'Notes / Payment Terms'}</p>
            <textarea className="w-full bg-transparent outline-none text-sm resize-none" style={{color: '#1C1C1E'}} rows={2} value={note} onChange={e => setNote(e.target.value)} />
          </div>
        </div>

        {!profile?.company_name && (
          <div className="rounded-2xl px-4 py-3 mb-4" style={{backgroundColor: '#FFF8E6', border: '1px solid #FF9F0A'}}>
        ...
        <div className="space-y-3">
          <button
            ...
            disabled={sending}
            className="w-full py-3.5 rounded-2xl text-sm font-semibold disabled:opacity-50 transition-opacity"
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className="py-3 rounded-2xl text-sm font-medium disabled:opacity-50 transition-opacity"
              style={{backgroundColor: '#FFFFFF', color: '#1C1C1E', border: '1px solid #E5E5EA'}}
            >
              {linkCopied ? '✅ ' + (lang === 'zh' ? '已复制' : 'Copied') : copyingLink ? '...' : '🔗 ' + (lang === 'zh' ? '复制链接' : 'Copy Link')}
            </button>
            <button
              onClick={() => window.print()}
              className="py-3 rounded-2xl text-sm font-medium transition-opacity"
              style={{backgroundColor: '#FFFFFF', color: '#1C1C1E', border: '1px solid #E5E5EA'}}
            >
