          ...
          {type === 'material' && (
            <>
              ...
              <div className="flex gap-3">
                ...
                <div className="w-24"><label className="text-gray-700 text-sm font-medium">{t.unit}</label><select className="w-full border border-gray-200 dark:border-[#3A3A3C] rounded-lg p-3 mt-1 text-gray-900 dark:text-[#F2F2F7] dark:bg-[#3A3A3C] outline-none" value={unit} onChange={(e) => setUnit(e.target.value)}>{unit && !['sqm','qbm','L','KL','KG','LM','EA'].includes(unit) && <option value={unit}>{unit}</option>}<option value="">--</option><option value="sqm">sqm 平方米</option><option value="qbm">qbm 立方米</option><option value="L">L 升</option><option value="KL">KL 千升</option><option value="KG">KG 千克</option><option value="LM">LM 米</option><option value="EA">EA 个</option></select></div>
              </div>
