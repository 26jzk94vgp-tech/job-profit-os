    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#1C1C1E]">
      <nav className="bg-white border-b border-gray-200 px-6 py-4 hidden md:block">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-semibold text-gray-900 dark:text-[#F2F2F7]">{job.name}</h1>
          </div>
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-4">
          <p className="text-gray-500 dark:text-[#8E8E93] dark:text-[#636366]">{job.client_name}</p>
        ...
        {revenue > 0 && (
          <div className="bg-white rounded-xl border border-gray-200 p-5 mb-4">
            <div className="flex justify-between items-center mb-2">
              ...
              <p className="text-sm font-medium text-gray-700 dark:text-[#EBEBF5]">
        ...
        <div className="bg-white rounded-xl border border-gray-200 mb-6">
          <div className="px-6 py-4 border-b border-gray-100 dark:border-[#3A3A3C]">
            <h2 className="font-semibold text-gray-900 dark:text-[#F2F2F7]">{lang === 'zh' ? '成本明细' : 'Cost Breakdown'}</h2>
          </div>
          <div className="divide-y divide-gray-100">
            <div className="px-6 py-3 flex justify-between"><span className="text-gray-600 dark:text-[#8E8E93] dark:text-[#636366]">{lang === 'zh' ? '人工' : 'Labor'}</span><span className="text-red-500">${labor.toLocaleString()}</span></div>
            <div className="px-6 py-3 flex justify-between"><span className="text-gray-600 dark:text-[#8E8E93] dark:text-[#636366]">{lang === 'zh' ? '材料' : 'Materials'}</span><span className="text-red-500">${material.toLocaleString()}</span></div>
            <div className="px-6 py-3 flex justify-between"><span className="text-gray-600 dark:text-[#8E8E93] dark:text-[#636366]">{lang === 'zh' ? '分包' : 'Subcontract'}</span><span className="text-red-500">${subcontract.toLocaleString()}</span></div>
            {fuel > 0 && <div className="px-6 py-3 flex justify-between"><span className="text-gray-600 dark:text-[#8E8E93] dark:text-[#636366]">{lang === 'zh' ? '车辆/油费' : 'Vehicle/Fuel'}</span><span className="text-red-500">${fuel.toLocaleString()}</span></div>}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 mt-4">
          ...
          <div className="px-6 py-4">
            {editingNotes ? (
              ...
              <p className="text-gray-600 text-sm">{notes || <span className="text-gray-400 dark:text-[#636366]">{lang === 'zh' ? '暂无备注，点击编辑添加' : 'No notes yet. Click Edit to add.'}</span>}</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 dark:border-[#3A3A3C]">
          <div className="px-6 py-4 border-b border-gray-100 dark:border-[#3A3A3C]">
            <h2 className="font-semibold text-gray-900 dark:text-[#F2F2F7]">{lang === 'zh' ? '条目' : 'Entries'}</h2>
          </div>
          {!entries.length && <div className="px-6 py-8 text-center text-gray-400 dark:text-[#636366]">{lang === 'zh' ? '还没有条目。' : 'No entries yet.'}</div>}
          <div className="divide-y divide-gray-100">
        ...
        <div className="bg-white rounded-xl border border-gray-200 dark:border-[#3A3A3C]">
          <div className="px-6 py-4 border-b border-gray-100 dark:border-[#3A3A3C]">
            <h2 className="font-semibold text-gray-900 dark:text-[#F2F2F7]">{lang === 'zh' ? '发票条目' : 'Invoice Entries'}</h2>
          </div>
          {entries.filter((e: any) => e.type === 'invoice').length === 0 && (
            <div className="px-6 py-8 text-center text-gray-400 dark:text-[#636366]">
