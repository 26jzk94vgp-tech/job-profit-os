...
export default function AddEntry({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleScan(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = async () => {
      ...
      } else { alert('Could not read receipt') }
      setScanning(false)
    }
      ...
      <main className="max-w-2xl mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex gap-2 mb-6">
            {tabs.map((t) => (
              <button key={t} onClick={() => setType(t)} className={t === type ? 'flex-1 py-2 rounded-lg text-sm font-medium capitalize bg-blue-600 text-white' : 'flex-1 py-2 rounded-lg text-sm font-medium capitalize bg-gray-100 text-gray-600'}>{t}</button>
            ))}
          </div>
          <p className="text-gray-400 text-xs mb-4">Tip: Use / for unknown values (e.g. TBD amounts)</p>
