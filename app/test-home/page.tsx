...
const FEED=[
  ...
  {id:5,cat:'☕',catEn:'Coffee',icon:'☕',title:'附近咖啡 $5 flat white',titleEn:'$5 flat white nearby',desc:'Northbridge Café 今日特惠，工地附近 3 分钟',descEn:'3 min from your site, today only',color:'#A0522D'},
  ...
  {id:6,cat:'新闻',catEn:'News',icon:'📰',title:'RBA 维持利率不变',titleEn:'RBA holds interest rate',desc:'澳洲央行今日宣布利率维持 4.35%',descEn:'Rate held at 4.35%, mortgage relief ahead',color:'#6E7681'},
]
...
function WelcomeScreen({T,lang,onStart}:{T:Theme,lang:'en'|'zh',onStart:()=>void}){
  return(
    <div style={{flex:1,display:'flex',flexDirection:'column' as const,alignItems:'center',justifyContent:'center',padding:'40px 24px',textAlign:'center' as const,minHeight:'70vh'}}>
      ...
      <button onClick={onStart} style={{padding:'14px 40px',borderRadius:'10px',border:'none',background:`linear-gradient(135deg,${T.primary},#58A6FF)`,color:'white',fontSize:'16px',fontWeight:600,cursor:'pointer',boxShadow:`0 4px 16px ${T.primaryGlow}`,marginBottom:'16px'}}>
        {lang==='en'?'Get Started →':'开始设置 →'}
      </button>
      <p style={{fontSize:'13px',color:T.textDim}}>{lang==='en'?'Already have an account? ':'已有账号？'}<span style={{color:T.primary,cursor:'pointer',fontWeight:500}}>{lang==='en'?'Sign in':'登录'}</span></p>
...
function OnboardingScreen({T,lang,onDone}:{T:Theme,lang:'en'|'zh',onDone:(data:any)=>void}){
  ...
  function canNext(){if(step===1)return!!industry;if(step===2)return!!size;if(step===3)return company.trim().length>0;return true}

  function GridBtn({item}:{item:{key:string,en:string,zh:string}}){
    const selected=industry===item.key
    return(
      <button onClick={()=>setIndustry(item.key)} style={{padding:'14px 10px',borderRadius:'8px',cursor:'pointer',border:`2px solid ${selected?T.primary:T.border}`,backgroundColor:selected?(T===LIGHT?'#EBF5FF':'rgba(47,129,247,0.1)'):T.surface,textAlign:'center' as const,transition:'all 0.15s'}}>
  ...
  return(
    <div style={{flex:1,maxWidth:'480px',margin:'0 auto',padding:'32px 24px 60px',width:'100%'}}>
      ...
      {step===2&&(<div>
        ...
        <div style={{display:'flex',flexDirection:'column' as const,gap:'10px',marginBottom:'28px'}}>
          {SIZES.map(s=>{const sel=size===s.key;return(
            <button key={s.key} onClick={()=>setSize(s.key)} style={{padding:'14px 18px',borderRadius:'8px',cursor:'pointer',border:`2px solid ${sel?T.primary:T.border}`,backgroundColor:sel?(T===LIGHT?'#EBF5FF':'rgba(47,129,247,0.1)'):T.surface,textAlign:'left' as const,display:'flex',alignItems:'center',justifyContent:'space-between',transition:'all 0.15s'}}>
      ...
      {step===4&&(<div style={{textAlign:'center' as const,paddingTop:'20px'}}>
        ...
        <button onClick={()=>onDone({industry,size,company,abn})} style={{width:'100%',padding:'14px',borderRadius:'8px',border:'none',background:`linear-gradient(135deg,${T.primary},#58A6FF)`,color:'white',fontSize:'15px',fontWeight:600,cursor:'pointer',marginBottom:'10px'}}>
...
function DashboardScreen({T,lang,isDark,onToggleDark,weather,userName}:{T:Theme,lang:'en'|'zh',isDark:boolean,onToggleDark:()=>void,weather:WeatherData,userName:string}){
  ...
  const navItems=[
    {label:zh?'概览':'Overview',icon:'🏠'},{label:zh?'报价单':'Quotes',icon:'📋'},
    {label:zh?'工单':'Jobs',icon:'🔨'},{label:zh?'财务':'Finance',icon:'💰'},
    {label:zh?'税务':'Tax',icon:'📊'},{label:zh?'客户':'Clients',icon:'👥'},
      ...
      <div style={{position:'sticky',top:0,zIndex:50,backgroundColor:T.surface,borderBottom:`1px solid ${T.border}`,height:'70px',display:'flex',alignItems:'center',paddingLeft:'20px'}}>
        ...
        <div style={{flex:1,display:'flex',alignItems:'center',gap:'16px',minWidth:0}}>
          ...
          <div style={{minWidth:0}}>
            <p style={{fontSize:'20px',fontWeight:700,color:T.text,margin:'0 0 4px'}}>{greeting}, {userName||'Shu'}</p>
        ...
        <div style={{flex:1,minWidth:0,padding:'20px 20px 60px'}}>
          <Section title={zh?'进行中工单':'Active Jobs'} dot={T.primary} count={MOCK_JOBS.length} T={T}
            action={<div style={{display:'flex',alignItems:'center',gap:'8px'}}>
              <button style={{fontSize:'11px',fontWeight:600,padding:'4px 10px',borderRadius:'5px',border:`1px solid ${T.primary}`,backgroundColor:T.primaryGlow,color:T.primary,cursor:'pointer'}}>{zh?'+ 新工单':'+ New Job'}</button>
              <span style={{fontSize:'11px',color:T.primary,cursor:'pointer',fontWeight:500}}>{zh?'全部 →':'All →'}</span>
            </div>}>
                ...
                <tbody>
                  {MOCK_JOBS.map((job,i)=>(
                    <tr key={job.id} style={{borderTop:i>0?`1px solid ${T.borderSub}`:'none',cursor:'pointer'}}
                      onMouseEnter={e=>(e.currentTarget.style.backgroundColor=T.elevated)}
                      onMouseLeave={e=>(e.currentTarget.style.backgroundColor='transparent')}>
                      <td style={{padding:'11px 16px',fontSize:'15px',fontWeight:500,color:T.primary}}>{job.name}</td>
                      <td style={{padding:'11px 16px',fontSize:'14px',color:T.textSub}}>{job.client}</td>
                      <td style={{padding:'11px 16px',fontSize:'15px',fontWeight:600,color:job.revenue>0?T.success:T.textDim,fontFamily:T.mono}}>{job.revenue>0?'+$'+job.revenue.toLocaleString():'—'}</td>
                      <td style={{padding:'11px 16px',fontSize:'14px',color:job.daysLeft<=5?T.danger:T.textDim,fontFamily:T.mono}}>{job.daysLeft<=5?`⚠ ${job.daysLeft}d`:`${job.daysLeft}d`}</td>
          ...
          <Section title={zh?'报价单':'Quotes'} dot={T.primary} count={MOCK_QUOTES.length} T={T}
            action={<div style={{display:'flex',alignItems:'center',gap:'8px'}}>
              <button style={{fontSize:'11px',fontWeight:600,padding:'4px 10px',borderRadius:'5px',border:`1px solid ${T.primary}`,backgroundColor:T.primaryGlow,color:T.primary,cursor:'pointer'}}>{zh?'+ 新报价单':'+ New Quote'}</button>
              <span style={{fontSize:'11px',color:T.primary,cursor:'pointer',fontWeight:500}}>{zh?'进入模块 →':'View all →'}</span>
            </div>}>
            ...
            {MOCK_QUOTES.map(q=>(
              <div key={q.id} style={{padding:'10px 16px',borderTop:`1px solid ${T.borderSub}`,display:'flex',alignItems:'center',justifyContent:'space-between',cursor:'pointer'}}
                ...
                <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                  <span style={{fontSize:'13px',color:T.textDim,fontFamily:T.mono,width:'44px'}}>{q.num}</span>
                  <div>
                    <p style={{fontSize:'15px',fontWeight:500,color:T.text,margin:'0 0 1px'}}>{q.client}</p>
                    <p style={{fontSize:'11px',color:T.textDim,margin:0}}>{q.date}</p>
                  </div>
                </div>
                <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                  <span style={{fontSize:'15px',fontWeight:600,color:T.success,fontFamily:T.mono}}>${q.amount.toLocaleString()}</span>
        ...
        <div className="hidden md:block" style={{width:`${PANEL_W}px`,flexShrink:0,borderLeft:`1px solid ${T.border}`,backgroundColor:T.surface,position:'sticky',top:'70px',height:'calc(100vh - 70px)',overflowY:'auto'}}>
          ...
          <div style={{margin:'10px',backgroundColor:T.bg,borderRadius:'4px',border:`1px solid ${T.border}`,height:'120px',position:'relative',overflow:'hidden'}}>
            ...
            {[{x:'28%',y:'38%',c:T.primary},{x:'63%',y:'52%',c:T.warning},{x:'43%',y:'67%',c:T.success},{x:'17%',y:'58%',c:T.textSub}].map((p,i)=>(
              <div key={i} style={{position:'absolute',left:p.x,top:p.y,transform:'translate(-50%,-50%)'}}><PulseDot color={p.c} size={8}/></div>
            ))}
...
export default function TestComplete() {
  ...
  return (
    <div style={{minHeight:'100vh',backgroundColor:T.bg,fontFamily:'-apple-system,BlinkMacSystemFont,"SF Pro Display",sans-serif',color:T.text,transition:'background 0.2s'}}>
      ...
      {screen === 'welcome' && <WelcomeScreen T={T} lang={lang} onStart={()=>setScreen('onboarding')}/>}
      {screen === 'onboarding' && <OnboardingScreen T={T} lang={lang} onDone={(data)=>{setUserName(data.company||'Shu');setScreen('dashboard')}}/>}
      {screen === 'dashboard' && <DashboardScreen T={T} lang={lang} isDark={isDark} onToggleDark={()=>setIsDark(!isDark)} weather={weather} userName={userName||'Shu'}/>}
    </div>
  )
}
