    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200">
            ...
            {sortJobs(jobs.filter((j: any) => ['active', 'paused'].includes(j.status))).map((job: any) => {
              ...
              return (
                <Link href={"/jobs/" + job.id} key={job.id}>
                  <div className="px-6 py-4 flex justify-between items-center hover:bg-gray-50 dark:hover:bg-[#3A3A3C] transition">
                    <div>
                      <p className="font-medium text-gray-900">{job.name}</p>
          ...
          {sortJobs(jobs.filter((j: any) => j.status === 'completed')).length > 0 && (
            <>
              ...
              {sortJobs(jobs.filter((j: any) => j.status === 'completed'), true).map((job: any) => {
                ...
                return (
                  <Link href={"/jobs/" + job.id} key={job.id}>
                    <div className={`px-6 py-4 flex justify-between items-center hover:bg-gray-50 dark:hover:bg-[#3A3A3C] transition ${hasUnpaid ? 'border-l-4 border-red-400' : 'opacity-80'}`}>
                      <div>
                        <p className="font-medium text-gray-900">{job.name}</p>
                        <p className="text-gray-500 text-sm">{job.client_name}</p>
          ...
          <Link href="/archive" className="flex items-center justify-between px-6 py-4 bg-gray-50 hover:bg-gray-100 dark:hover:bg-[#3A3A3C] border-t border-gray-200 rounded-b-xl transition">
            <div className="flex items-center gap-3">
