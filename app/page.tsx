                        ...
                        return(
                          <tr key={job.id} style={{borderTop:i>0?`1px solid ${T.borderSub}`:'none',cursor:'pointer'}}
                            ...
                            <td style={{padding:'11px 16px'}}>
                              <Link href={`/jobs/${job.id}`} style={{fontSize:'15px',fontWeight:500,color:T.primary,textDecoration:'none'}}>{job.name.replace(/\s*的工单\s*$/,'')}</Link>
                            </td>
                            <td style={{padding:'11px 16px'}}>
                              ...
                              {daysLeft!==null&&<span style={{fontSize:'12px',fontWeight:600,color:isUrgent?T.danger:T.textDim,fontFamily:T.mono}}>{isUrgent?'▲ ':''}{daysLeft}d left</span>}
                            </td>
                            <td style={{padding:'11px 16px'}}>
                              <StatusDropdown value={meta.mat} options={MAT_OPTIONS} onChange={s=>setJobMeta(m=>({...m,[job.id]:{...m[job.id],mat:s}}))} T={T} isZh={zh}/>
                            </td>
...
+Number(job.revenue).toLocaleString():'—'}
                            ...
                            <td style={{padding:'11px 16px'}}>
                              <Badge label={daysLeft===null?(job.status==='active'?(zh?'进行中':'Active'):(zh?'新建':'New')):daysLeft<0?(zh?`已超期${Math.abs(daysLeft)}天`:`${Math.abs(daysLeft)}d overdue`):daysLeft===0?(zh?'今天截止':'Due today'):(zh?`还有${daysLeft}天`:`${daysLeft}d left`)} type={daysLeft!==null&&daysLeft<=0?'danger':daysLeft!==null&&daysLeft<=5?'warning':'muted'} T={T} isZh={zh}/>
                            </td>
                          </tr>
                        )
                      })}
