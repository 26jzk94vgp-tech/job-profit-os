...
const FEED=[
  ...
  {id:3,cat:'工期',catEn:'Jobs',icon:'⚡',title:'2 个工地本周截止',titleEn:'2 sites due this week',desc:'kk (2d) · jn (5d)',descEn:'kk (2d) · jn (5d)',color:'#D29922'},
  {id:4,cat:'餐厅',catEn:'Food',icon:'🍜',title:'附近餐厅午市优惠',titleEn:'Nearby lunch deals',desc:'Northbridge 3 家 $12起',descEn:'3 restaurants from $12',color:'#FF6B6B'},
  {id:5,cat:'新闻',catEn:'News',icon:'📰',title:'RBA 维持利率不变',titleEn:'RBA holds interest rate',desc:'澳洲央行今日宣布利率维持 4.35%',descEn:'Rate held at 4.35%',color:'#6E7681'},
]
                ...
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  <thead>
                    <tr style={{backgroundColor:T.bg}}>
                      {[zh?'工单':'Job',zh?'客户':'Client',zh?'收入':'Revenue',zh?'状态':'Status'].map(h=>(
                  ...
                  <tbody>
                    {activeJobs.map((job,i)=>(
                      <tr key={job.id} style={{borderTop:i>0?`1px solid ${T.borderSub}`:'none',cursor:'pointer'}}
                        ...
                        <td style={{padding:'11px 16px',fontSize:'14px',color:T.textSub}}>{job.client_name||'—'}</td>
