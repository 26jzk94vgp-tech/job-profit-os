      ...
      <main className="max-w-4xl mx-auto px-6 py-6 space-y-4">
            ...
            <div className="divide-y divide-gray-100">
              {centsPerKm.map((e: any) => (
                <div key={e.id} className="px-6 py-4 flex justify-between items-start">
                  <div>
                    <p className="text-sm font-medium text-gray-900 dark:text-white">{e.trip_from} → {e.trip_to}</p>
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <div className="divide-y divide-gray-100">
              {actualCost.map((e: any) => (
                <div key={e.id} className="px-6 py-4 flex justify-between items-start">
                  <div>
                    <p className="text-sm font-medium text-gray-900 dark:text-white">{e.trip_from || (lang === 'zh' ? '油费' : 'Fuel')} {e.trip_to ? '→ ' + e.trip_to : ''}</p>
                    <p className="text-gray-400 text-xs">{formatDate(e.entry_date || e.created_at)} · {e.jobs?.name}</p>
                  </div>
                  <span className="text-green-600 font-medium text-sm">${Number(e.amount).toFixed(2)}</span>
                </div>
        ...
        {entries.length === 0 && (
          <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400 dark:text-gray-500">{lang === 'zh' ? '还没有行程记录' : 'No vehicle trips recorded yet.'}</p>
